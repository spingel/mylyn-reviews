...
public class AddOrEditFishEyeMappingDialog extends ProgressDialog {
	...
	protected Control createPageControls(Composite parent) {
		...
		GridDataFactory.fillDefaults()
				...
				.applyTo(updateServerDataButton);
		...
		if (taskRepositories == null || taskRepositories.size() == 0) {
			...
			setMessage("Mapping cannot be defined. FishEye or Crucible server must be defined first.",
...
//			// job interrupted by user so ignore it
			...
			StatusHandler.log(new Status(IStatus.ERROR, AtlassianTeamUiPlugin.PLUGIN_ID,
					"Failed to update task repository details", e));
		} finally {
