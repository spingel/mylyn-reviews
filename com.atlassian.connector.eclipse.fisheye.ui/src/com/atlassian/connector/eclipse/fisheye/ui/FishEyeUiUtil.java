...
//				}
	...
	public static void setScmRepositoryMappings(@NotNull Collection<FishEyeMappingConfiguration> mappings)
			throws IOException {
...
//			}
	...
	@NotNull
	public static String buildFishEyeUrl(@NotNull IResource resource, @Nullable LineRange lineRange)
			throws CoreException, NoMatchingFishEyeConfigurationException {
		final LocalStatus revisionInfo = TeamUiUtils.getLocalRevision(resource);
		if (revisionInfo == null || !revisionInfo.isVersioned()) {
			throw new CoreException(new Status(IStatus.ERROR, AtlassianTeamUiPlugin.PLUGIN_ID,
		...
		if (cfg == null) {
			throw new NoMatchingFishEyeConfigurationException(new Status(IStatus.ERROR,
					AtlassianTeamUiPlugin.PLUGIN_ID, "Cannot find matching FishEye repository for " + revisionInfo),
					scmPath);
		}
...
//			if (taskRepository.getConnectorKind().equals(AtlassianTeamUiPlugin.CONNECTOR_KIND)
